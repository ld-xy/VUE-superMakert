<template>
    <div id="detail">
        ...
        <back-top v-show='showTop' @click.native="backTop"></back-top> <!--接收子代传来的绑定事件，但是不能直接监听事件-->
...
export default {
  ...
  // 还可将防抖函数保存在data中，就可使得组件中任意位置都能使用防抖函数,不然只能在mounted钩子函数使用
  ...
  methods: {
    ...
    // 封装到混入mixin里
      ...
      // }
    addToCart () {
      // 1.获取购物车需要展示的信息，因为有多个信息，所以可以放在一个对象里  length push pop find reverse sort max min
      const product = {}
      ...
      // 2.将商品添加到购物车里
      // this.$store.commit('addCart',product) commit是将product提交到store中mutations里的方法addCart 直接提交
      this.$store.dispatch('addCart',product)  // dispatch是提交到actions里去，异步提交，再由actions分发commit到mutations里不同的方法中去
      // 3.如果添加购物车成功（用promise监视成功与否）  此处还有一种方法就是使用mapActions工具映射action函数到这个vue中，可以使用this直接调函数
      .then(res => {
        // console.log(res);
        // this.isShow = true
        // this.message = res
